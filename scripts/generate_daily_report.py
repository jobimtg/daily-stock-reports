"""
Daily Stock Report Generator (v4 - multi-region, stable Gemini JSON + date cache)
----------------------------------------------------------------------------------
...
請只回傳有效 JSON，不要 markdown，不要解釋，不要附註。JSON 格式如下：
{{
...
def _extract_gemini_json(text):
    """
    Gemini sometimes returns markdown fences or extra notes.
    This extracts the first JSON object between the first { and last }.
    """
    text = (text or "").strip()
...
def _gemini_cache_file(region, report_date):
    """
    Date-based cache.
    This prevents repeated manual runs on the same date from consuming Gemini quota.
    """
...
def call_gemini(prompt, region, report_date):
    ...
    for attempt, wait_seconds in enumerate(retry_waits, start=1):
        ...
        try:
            ...
            print(f"[warn] Gemini returned invalid JSON on attempt {attempt}: {e}", file=sys.stderr)
